...
- ~~Decisión de negocio pendiente sobre "Equinox RC" (`docs/02.10-rc-accidentes-conductor.md`): GNP rechaza Accidentes al Conductor sobre Responsabilidad Civil — aceptar el hueco, mover la plantilla a un paquete base distinto (Limitada) que sí la trae, u ofrecerla aparte.~~ **Cerrado 2026-09-11, decisión de Beto**: el hueco en RC se queda **permanente, no temporal** — no hace falta mover ni ofrecer nada aparte. Accidentes al Conductor ya está incluida en "Equinox Limitada" desde el Excel original de Beto (una de sus 7 coberturas desde el principio, nunca rechazada): el negocio ya había resuelto esto sin saberlo al definir el contenido de las 4 plantillas reales. Un cliente que la necesite se cotiza con Limitada en vez de con RC/Básica. Verificado contra la base real: `cat_plantilla_coberturas` de la plantilla Limitada (`id = 77`) sí trae `0000000893` (Accidentes al Conductor, suma 100,000). No requirió ningún cambio de datos ni de código — las plantillas ya cargadas reflejan correctamente esta decisión tal como están.
...
- [`docs/02.13-armador-libre-backend.md`](../02.13-armador-libre-backend.md) — armador libre de coberturas, Fase 1 (sólo backend): combinar coberturas ad-hoc sobre un paquete base, cotizar puntual sin tocar `cat_plantillas`, guardar como plantilla nueva opcional. Probado contra producción; sin pantalla todavía
- [`docs/02.14-armador-libre-pantallas.md`](../02.14-armador-libre-pantallas.md) — armador libre, Fase 2: pantalla `armador.php` (Personalizar una plantilla / Armar desde cero), editor de coberturas de `plantillas.php` movido a componente compartido, probado por la pantalla real contra producción. Corrige `ADR-005` punto 6 (clave 37 es genérica)
